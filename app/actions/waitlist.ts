...
export async function submitWaitlist(
  ...
  const raw = {
    ...
    cuisines: formData.getAll('cuisines').map((v) => v.toString()),
    cuisine_other: (formData.get('cuisine_other') ?? '').toString(),
  ...
  try {
    ...
    const { error } = await supabase.from('seller_waitlist').insert({
      ...
      cuisine_other: data.cuisine_other ? data.cuisine_other : null,
